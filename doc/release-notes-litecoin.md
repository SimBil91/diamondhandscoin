Hubucoin Core version 0.18.1 is now available from:

  <https://download.hubucoin.org/hubucoin-0.18.1/>
...
`/Applications/Hubucoin-Qt` (on Mac) or `hubucoind`/`hubucoin-qt` (on
Linux).
...
Hubucoin Core is supported and extensively tested on operating systems
using the Linux kernel, macOS 10.10+, and Windows 7 and newer. It is not
recommended to use Hubucoin Core on unsupported systems.
...
10.10. Additionally, Hubucoin Core does not yet change appearance when
...
- Four new options are available for configuring the maximum number of
  ...
  documentation](https://github.com/hubucoin-project/hubucoin/blob/master/doc/zmq.md#usage)
  for details.
...
- The `whitelistforcerelay` option is used to relay transactions from
  ...
  [contacting](mailto:contact@hubucoin.org) the Hubucoin Core
  project to let us know about their use-case, as this feature could be
  deprecated in the future).

Wallet changes
--------------
When creating a transaction with a fee above `-maxtxfee` (default 0.1 HUC),
the RPC commands `walletcreatefundedpsbt` and  `fundrawtransaction` will now fail
instead of rounding down the fee. Beware that the `feeRate` argument is specified
in HUC per kilobyte, not litoshi per byte.

Documentation
-------------

- A new short [document](https://github.com/hubucoin-project/hubucoin/blob/master/doc/JSON-RPC-interface.md)
  about the JSON-RPC interface describes cases where the results of an
  RPC might contain inconsistencies between data sourced from different
  subsystems, such as wallet state and mempool state.  A note is added
  to the [REST interface documentation](https://github.com/hubucoin-project/hubucoin/blob/master/doc/REST-interface.md)
  indicating that the same rules apply.

- Further information is added to the [JSON-RPC
  documentation](https://github.com/hubucoin-project/hubucoin/blob/master/doc/JSON-RPC-interface.md)
  about how to secure this interface.

- A new [document](https://github.com/hubucoin-project/hubucoin/blob/master/doc/hubucoin-conf.md)
  about the `hubucoin.conf` file describes how to use it to configure
  Hubucoin Core.

- A new document introduces Hubucoin Core's BIP174 [Partially-Signed
  Hubucoin Transactions
  (PSBT)](https://github.com/hubucoin-project/hubucoin/blob/master/doc/psbt.md)
...
- The [output script
  descriptor](https://github.com/hubucoin-project/hubucoin/blob/master/doc/descriptors.md)
...
- A new `--disable-bip70` option may be passed to `./configure` to
  prevent Hubucoin-Qt from being built with support for the BIP70 payment
  protocol or from linking libssl.  As the payment protocol has exposed
  Hubucoin Core to libssl vulnerabilities in the past, builders who don't
...
- The minimum required version of Qt (when building the GUI) has been
  ...
  system](https://github.com/hubucoin-project/hubucoin/blob/master/depends/README.md)
  provides 5.9.7)
...
- `deriveaddresses` returns one or more addresses corresponding to an
  [output descriptor](https://github.com/hubucoin-project/hubucoin/blob/master/doc/descriptors.md).
...
- `getaddressinfo` now returns an additional `solvable` boolean field
  when Hubucoin Core knows enough about the address's scriptPubKey,
...
- `importprivkey` will preserve previously-set labels for addresses or
  ...
  wallet" in earlier releases of Hubucoin Core, subsequently importing
...
- `importmulti` now supports importing of addresses from descriptors. A
  ...
  [here](https://github.com/hubucoin-project/hubucoin/blob/master/doc/descriptors.md).
...
- The 'account' API is removed after being deprecated in v0.17.  The
  ...
  v0.17](https://github.com/hubucoin-project/hubucoin/blob/master/doc/release-notes/release-notes-0.17.0.md#label-and-account-apis-for-wallet)
...
- `generate` is deprecated and will be fully removed in a subsequent
  ...
  in this version, restart hubucoind with the `-deprecatedrpc=generate`
...
- A new `hubucoin-wallet` tool is now distributed alongside Hubucoin
...
This section describes planned changes to Hubucoin Core that may affect
other Hubucoin software and services.

- Since version 0.16.0, Hubucoin Core’s built-in wallet has defaulted to
  ...
  widely-used software.  Starting with Hubucoin Core 0.20 (expected about
  a year after 0.18), Hubucoin Core will default to native segwit
  ...
  to bech32 addresses, and if the Hubucoin Core project sees enough
  additional adoption, it will instead default to bech32 receiving
  addresses in Hubucoin Core 0.20.
  ...
  set the `addresstype=bech32` configuration option in any Hubucoin Core
...
- This release allows peers that your node automatically disconnected
  ...
  which you don't have many other peers).  Previously, Hubucoin Core
...
- A sub-project of Hubucoin Core now provides Hardware Wallet Interaction
  (HWI) scripts that allow command-line users to use several popular
  hardware key management devices with Hubucoin Core.  See their [project
...
- This release changes the Random Number Generator (RNG) used from
  OpenSSL to Hubucoin Core's own implementation, although entropy
  gathered by Hubucoin Core is fed out to OpenSSL and then read back in
  when the program needs strong randomness. This moves Hubucoin Core a
...
- On macOS, Hubucoin Core now opts out of application CPU throttling

...
* mempoolminfee : (numeric) minimum feerate (HUC per KB) for tx to be accepted
...
Risks
-------------
Running a web browser on the same node with a REST enabled hubucoind can be a risk. Accessing prepared XSS websites could read out tx/block data of your node by placing links like `<script src="http://127.0.0.1:9332/rest/tx/1234567890.json">` which might break the nodes privacy.

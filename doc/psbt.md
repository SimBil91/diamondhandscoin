# PSBT Howto for Hubucoin Core

Since Hubucoin Core 0.17, an RPC interface exists for Partially Signed Hubucoin
...
PSBT is an interchange format for Hubucoin transactions that are not fully signed
...
#### Multisig with multiple Hubucoin Core instances

Alice, Bob, and Carol want to create a 2-of-3 multisig address. They're all using
Hubucoin Core. We assume their wallets only contain the multisig funds. In case
they also have a personal wallet, this can be accomplished through the
multiwallet feature - possibly resulting in a need to add `-rpcwallet=name` to
the command line in case `hubucoin-cli` is used.

Hubucoin Core version *0.15.1* is now available from:

  <https://download.hubucoin.org/hubucoin-0.15.1/>
...
A number of changes to the way Hubucoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 hubs per kB (0.00001 HUC). This is to prepare for dropping the minimum transaction fee to 0.1 hubs per kB (0.0001 HUC) in the next release.

DiamondHandsCoin Core version *0.15.1* is now available from:

  <https://download.diamondhandscoin.org/diamondhandscoin-0.15.1/>
...
A number of changes to the way DiamondHandsCoin Core deals with peer connections and invalid blocks
...
The minimum relay fee `-minrelayfee` has been lowered to 0.01 hubs per kB (0.00001 DHC). This is to prepare for dropping the minimum transaction fee to 0.1 hubs per kB (0.0001 DHC) in the next release.

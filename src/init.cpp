 ...
 */
static const char* BITCOIN_PID_FILENAME = "diamondhandscoind.pid";
...
void Shutdown(InitInterfaces& interfaces)
{
    ...
    RenameThread("diamondhandscoin-shutoff");
...
{
    const std::string URL_SOURCE_CODE = "<https://github.com/diamondhandscoin-project/diamondhandscoin>";
    const std::string URL_WEBSITE = "<https://diamondhandscoin.org>";
...
static void ThreadImport(std::vector<fs::path> vImportFiles)
{
    ...
    RenameThread("diamondhandscoin-loadblk");
...
bool AppInitMain(InitInterfaces& interfaces)
{
    ...
    if (gArgs.IsArgSet("-datadir") && !fs::path(gArgs.GetArg("-datadir", "")).is_absolute()) {
        LogPrintf("Warning: relative datadir option '%s' specified, which will be interpreted relative to the " /* Continued */
                  "current working directory '%s'. This is fragile, because if diamondhandscoin is started in the future "
                  "from a different location, it will be unable to locate the current data files. There could "
                  "also be data loss if diamondhandscoin is started while in a temporary directory.\n",
